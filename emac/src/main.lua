...
local function setColours(grayscale)
    if term.isColour() and not grayscale then
        ...
        bgColour = colours.black
        textColour = colours.white
        highlightColour = (grayscale and colours.yellow or colours.white)
...
local function load( _sPath )
    ...
    if #tLines == 0 then
        table.insert( tLines, "--!lua" )
        return
    end
    if tLines[1]=="--!text" then -- text shouldn't have highlighting or be ran
        setColours(true)
        tMenuItems = {}
        if not bReadOnly then
            table.insert( tMenuItems, "Save" )
    ...
    Save = function()
        if bReadOnly then
            ...
            if ok then
                ...
                sStatus="Error saving to "..sPath..": "..err
            end
        end
        redrawMenu()
    end,
